...
export default function Minimap({
  ...
  useEffect(() => {
    ...
    if (roverPath && roverPath.length >= 2) {
      ...
      ctx.beginPath();
      ctx.arc(
        (sx / WORLD_SPAN + 0.5) * MAP_SIZE,
        (sz / WORLD_SPAN + 0.5) * MAP_SIZE,
        3,
        0,
        Math.PI * 2,
      );
      ...
      ctx.beginPath();
      ctx.arc(
        (ex / WORLD_SPAN + 0.5) * MAP_SIZE,
        (ez / WORLD_SPAN + 0.5) * MAP_SIZE,
        3.5,
        0,
        Math.PI * 2,
      );
  ...
  return (
    <motion.div
      ...
      animate={
        isStarted ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.9 }
      }
    ...
    >
      ...
      <div className="mb-1.5 flex items-center justify-between px-1">
        <span className="text-[9px] font-bold tracking-[0.3em] text-cyan-300/80">
          ◈ MAP
        </span>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">
        ...
        <button
          ...
          className="rounded-md border border-cyan-300/30 bg-cyan-500/10 px-2 py-1 text-[9px] tracking-[0.18em] text-cyan-100 transition-colors hover:bg-cyan-500/20"
        >
          {isRoverFocused ? "SERBEST KAMERA" : "ROVERA YAKLAŞ"}
        </button>
      </div>
    </motion.div>
  );
