...
export default function Rover({
  ...
  const obstacles = useMemo(() => {
    const seed =
      mapId.split("").reduce((acc, ch) => acc + ch.charCodeAt(0), 0) + 1337;
  ...
  const ventRows = useMemo(() => [-0.18, -0.06, 0.06, 0.18], []);
  ...
  useFrame((state, delta) => {
    ...
    if (isPlaying) {
      if (hasRoute) {
        ...
        if (dist > 0.0001) {
          ...
          while (diff > Math.PI) diff -= Math.PI * 2;
          const turnSpeed = Math.min(1.0, delta * 3.5);
          motion.headingY = smoothStep(
            motion.headingY,
            motion.headingY + diff,
            turnSpeed,
          );
        }
        ...
        // Smooth and stable movement toward target
        motion.x = smoothStep(motion.x, desiredX, 0.2);
        motion.z = smoothStep(motion.z, desiredZ, 0.2);
    ...
    const hitShakeX =
      Math.sin(t * 92) * 0.018 * motion.impact * motion.impactSide;
    ...
    if (navRightRef.current)
      navRightRef.current.emissiveIntensity = navIntensity;
  });
  ...
  return (
    ...
    >
      ...
      <spotLight
        position={[0, 1.2, 0.9]}
        angle={0.6}
        intensity={headlightsOn ? 30 : 5}
        color="#fffcee"
        distance={45}
        decay={1.6}
        penumbra={0.55}
        castShadow={headlightsOn}
        target-position={[0, 0.1, 6]}
      />
