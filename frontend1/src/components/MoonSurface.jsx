...
export default function MoonSurface({
  ...
  const geometry = useMemo(() => {
    ...
    const fadeStart = planeSize * 0.32;
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      const edgeDark = Math.max(0.05, 1 - edgeFactor * 0.9);
  ...
  return (
    ...
    >
      ...
      <Rocks
        count={profile.rockCount}
        spread={planeSize * 0.88}
        mapId={selectedMap}
      />
      <Boulders
        count={profile.boulderCount}
        spread={planeSize * 0.82}
        mapId={selectedMap}
      />
    </group>
  );
